...
public class AuditeursVerfies extends JPanel {
	private JTable table;
	public  Listener l=null;
	/**
	 * Create the panel.
	 */
	public AuditeursVerfies() {
		...
		panelInfos.setBounds(0, 0, 701, 618);
		add(panelInfos);
		...
		Button buttonAverifier = new Button("A verifier");
		
	
		buttonAverifier.setForeground(Color.WHITE);
		buttonAverifier.setFont(new Font("MS Reference Sans Serif", Font.BOLD, 15));
		buttonAverifier.setBackground(new Color(30, 144, 255));
		buttonAverifier.setBounds(300, 500, 100, 35);
		panelInfos.add(buttonAverifier);
		
		JLabel invisibleid = new JLabel("");
		invisibleid.setVisible(false);
		invisibleid.setFont(new Font("MS Reference Sans Serif", Font.PLAIN, 15));
		invisibleid.setBounds(12, 440, 132, 47);
		...
		buttonAverifier.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				proxy.AverifierListener(l.getId_listener());
				JOptionPane.showMessageDialog(null, "cet auditeur est déplacé vers la table des auditeurs à verifier ","", JOptionPane.INFORMATION_MESSAGE);
			}
		});
